...
export default class TelegramBot {
	...
	constructor(token: string, options?: { defaultCommand?: string }) {
		...
		// Register the default handler for the default command to avoid errors
	...
	private determineCommand(ctx: TelegramExecutionContext, args: string[]): string {
		...
		// Check for bot commands using message entities
		if (ctx.update_type === 'message' || ctx.update_type === 'business_message') {
			const message = ctx.update_type === 'message' ? ctx.update.message : ctx.update.business_message;
			const entities = message?.entities;

			if (entities && entities.length > 0) {
				// Look for the bot_command entity at the beginning of the message
				const commandEntity = entities.find(entity =>
					entity.type === 'bot_command' && entity.offset === 0
				);

				if (commandEntity && message.text) {
					// Extract command text using the entity offset and length
					const commandText = message.text.substring(
						commandEntity.offset,
						commandEntity.offset + commandEntity.length
					);

					// Remove the leading '/' and any bot username (e.g., /start@botname -> start)
					const command = commandText.slice(1).split('@')[0];
		...
		// Fallback: check if it's a command starting with / (for backward compatibility)
		if (args.at(0)?.startsWith('/')) {
			const command = args.at(0)?.slice(1).split('@')[0] ?? '';
			return command in this.commands ? command : this.defaultCommand;
		}
	...
	private parseArguments(ctx: TelegramExecutionContext): string[] {
		switch (ctx.update_type) {
			...
			case 'business_message': {
				...
				// If there are entities and the first one is a bot_command,
				// parse arguments starting after the command
				const entities = message.entities;
				if (entities && entities.length > 0) {
					const commandEntity = entities.find(entity =>
						entity.type === 'bot_command' && entity.offset === 0
					...
					if (commandEntity) {
						// Get text after the command and split by spaces
						const afterCommand = text.substring(commandEntity.offset + commandEntity.length).trim();
						return afterCommand ? afterCommand.split(' ') : [];
					}
				}

				// Fallback to splitting the entire text
				return text.split(' ');
			}
			case 'inline':
